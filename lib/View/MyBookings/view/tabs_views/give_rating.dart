...
class PromotionRatingScreen extends StatefulWidget {
  final String bookingId;
  const PromotionRatingScreen(
      {Key? key, required this.bookingId, })
      : super(key: key);

  @override
  PromotionRatingScreenState createState() => PromotionRatingScreenState();
}

class PromotionRatingScreenState extends State<PromotionRatingScreen> {
  var isLoading = false;
  double rating = 3.0;
  ...
  giveRating() async {
    ...
    requestParams['promotionID'] = widget.bookingId;

    var categories = await RatingRepo().giveRatingToPromotionBooking(requestParams);
