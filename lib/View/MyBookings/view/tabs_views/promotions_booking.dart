                  ...
                  if (snapshot.hasData) {
                    return RefreshIndicator(
                      ...
                      child: ListView.builder(
                        itemCount: controller.isSearching == true ? controller.filteredPromotionBookingList!.length : snapshot.data?.result?.length,
                        padding: const EdgeInsets.symmetric(vertical: 5, horizontal: 5),
                        itemBuilder: (BuildContext contextM, index) {
                          var data =
                              controller.isSearching == false ? snapshot.data?.result![index] : controller.filteredPromotionBookingList![index];
                          return Padding(
                              ...
                              child: Column(mainAxisAlignment: MainAxisAlignment.start, children: [
                                Row(
                                  ...
                                  children: [
                                    ...
                                    Column(
                                      children: [
                                        imageWidget(imagePath: data?.promotion?.promoImg?.first.toString()),
                                        const SizedBox(
                                    ...
                                    Expanded(
                                      child: Column(
                                        children: [
                                          ...
                                          Row(
                                            children: [
                                              textwidget(text: "Promotion Title: ".tr, fontsize: 0, fontweight: 0),
                                              Flexible(
                                                child: textwidget(text: "${data?.promotion?.title}", fontsize: 0, fontweight: 0),
                                              ),
                                            ],
                                          ),
                                          Row(
                                                  ...
                                                  onTap: () {
                                                    if (data.status == "booked") {
                                                      Global.showToastAlert(
                                                          context: context,
                                                          strTitle: "Message",
                                                          strMsg: "Your booking is not completed yet",
                                                          toastType: TOAST_TYPE.toastSuccess);
                                                    } else {
                                                      displayTextInputDialog(context, data.sId!);
                                                    }
                                                  },
                                                  child: Container(
                                                      padding: const EdgeInsets.symmetric(horizontal: 6.0, vertical: 4.0),
                                                      decoration:
                                                          BoxDecoration(color: AppColors.colorYellowShade, borderRadius: BorderRadius.circular(2)),
                                                      child: Center(
                                                        child: Text(
                                                          "Give rating".tr.toUpperCase(),
                                              ...
                                              Expanded(
                                                child: InkWell(
                                                  ...
                                                  child: Container(
                                                      padding: const EdgeInsets.symmetric(horizontal: 6.0, vertical: 4.0),
                                                      decoration:
                                                          BoxDecoration(color: AppColors.colorPrimary, borderRadius: BorderRadius.circular(2)),
                                                      child: Center(
                                                        child: Text(
                                                          "${data.status}".tr.toUpperCase(),
