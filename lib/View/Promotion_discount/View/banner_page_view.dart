...
class BannerPageViewState extends State<BannerPageView> {
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint(
      "al promtion length" + widget.alPromotions.where((element) => element.location == "servicePage").length.toString(),
    );
    if (widget.alPromotions.isNotEmpty) {
      return LayoutBuilder(builder: (context, size) {
        return Directionality(
          ...
          child: SizedBox(
            height: MediaQuery.of(context).size.width > 600 ? size.maxWidth / 1.7 : size.maxWidth / 1.3,
            child: PageView.builder(
              ...
              itemBuilder: (context, index) {
                var promotionList = Get.put(DashboardController()).isHomePage == true
                ...
                return PromotionBanner(
                    ...
                    onTap: () {
                      Get.find<HomeScreenController>().promotionFragment(mPromotionsModel);
                    },
                    mPromotionsModel: mPromotionsModel,
                    strImage: mPromotionsModel.getPromoImage());
              },
              itemCount: Get.put(DashboardController()).isHomePage == true
