...
class MyProfileRepository {
  Future<Either<Failure, Success>> getCarList(
    ...
    try {
      String response = await ReqListener.fetchPost(
          strUrl: RequestBuilder.API_GET_CAR_LIST, requestParams: requestParams, mReqType: ReqType.get, mParamType: ParamType.simple);
      Result? mResponse;
      ...
      log("cars list :" + mResponse!.responseData.toString());
      ...
      if (mResponse?.responseStatus == true) {
        ...
        Success mSuccess = Success(responseStatus: mResponse!.responseStatus, responseData: carList, responseMessage: mResponse.responseMessage);
  ...
  Future<Either<Failure, Success>> addCar(HashMap<String, Object> requestParams) async {
    bool connectionStatus = await ConnectivityStatus.isConnected();
    if (!connectionStatus) {
      return Left(Failure(DATA: "", MESSAGE: AppAlert.ALERT_NO_INTERNET_CONNECTION, STATUS: false));
    }
    try {
      String response = await ReqListener.fetchPost(
          strUrl: RequestBuilder.API_ADD_CAR_LIST, requestParams: requestParams, mReqType: ReqType.post, mParamType: ParamType.json);
      Result? mResponse;
      if (response.isNotEmpty) {
        mResponse = Global.getData(response);
      } else {
  ...
  Future<Either<Failure, Success>> deletecar(HashMap<String, Object> requestParams, String id) async {
    bool connectionStatus = await ConnectivityStatus.isConnected();
    if (!connectionStatus) {
      return Left(Failure(DATA: "", MESSAGE: AppAlert.ALERT_NO_INTERNET_CONNECTION, STATUS: false));
    }
    try {
      String response = await ReqListener.fetchPost(
          strUrl: RequestBuilder.API_DELETE_CAR_ + id, requestParams: requestParams, mReqType: ReqType.delete, mParamType: ParamType.json);
      Result? mResponse;
      if (response.isNotEmpty) {
        mResponse = Global.getData(response);
      } else {
  ...
  Future<Either<Failure, Success>> updatecar(HashMap<String, Object> requestParams, String id) async {
    bool connectionStatus = await ConnectivityStatus.isConnected();
    if (!connectionStatus) {
      return Left(Failure(DATA: "", MESSAGE: AppAlert.ALERT_NO_INTERNET_CONNECTION, STATUS: false));
    }
    try {
      String response = await ReqListener.fetchPost(
          strUrl: RequestBuilder.API_DELETE_CAR_ + id, requestParams: requestParams, mReqType: ReqType.patch, mParamType: ParamType.json);
      Result? mResponse;
      if (response.isNotEmpty) {
        mResponse = Global.getData(response);
      } else {
  ...
  //------------------Send Emergency-------------------------------
  Future<Either<Failure, Success>> sendEmergency(HashMap<String, Object> requestParams) async {
    bool connectionStatus = await ConnectivityStatus.isConnected();
    if (!connectionStatus) {
      return Left(Failure(DATA: "", MESSAGE: AppAlert.ALERT_NO_INTERNET_CONNECTION, STATUS: false));
    }
    try {
      String response = await ReqListener.fetchPost(
          strUrl: RequestBuilder.API_EMERGENCY, requestParams: requestParams, mReqType: ReqType.post, mParamType: ParamType.json);
      Result? mResponse;
      if (response.isNotEmpty) {
        mResponse = Global.getData(response);
      } else {
