...
class TransactionRepo {
  Future<Either<Failure, Success>> getPurchaseHistory(
    ...
    try {
      String response = await ReqListener.fetchPost(
          strUrl: 'transactions?metadata.type=spent',
      ...
      if (mResponse?.responseStatus == true) {
        ...
        List<TransactionModel> alTransaction = [];
        for (var transaction in data) {
          alTransaction.add(TransactionModel.fromMap(transaction));
        }
        Success mSuccess = Success(
            responseStatus: mResponse!.responseStatus,
            responseData: alTransaction,
