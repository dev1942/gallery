...
class WalletRepo {
  ...
  Future<Either<Failure, Success>> getAnalyticsData(
    ...
    try {
      String response = await ReqListener.fetchPost(
          strUrl: RequestBuilder.API_GET_STATISTICS,
      ...
      if (mResponse?.responseStatus == true) {
        ...
        (data['walletBalance'] as Map<String, dynamic>).addAll({
  ...
  Future<Either<Failure, Success>> getWalletAndEarning(
      HashMap<String, Object> requestParams) async {
    bool connectionStatus = await ConnectivityStatus.isConnected();
    if (!connectionStatus) {
      return Left(Failure(
    ...
    try {
      String response = await ReqListener.fetchPost(
          strUrl: RequestBuilder.API_GET_WALLET,
          requestParams: requestParams,
          mReqType: ReqType.get,
          mParamType: ParamType.simple);
      Result? mResponse;
  ...
  Future<Either<Failure, Success>> addMoney(
      HashMap<String, Object> requestParams) async {
    bool connectionStatus = await ConnectivityStatus.isConnected();
    if (!connectionStatus) {
      return Left(Failure(
  ...
  Future<Either<Failure, Success>> addBankAccount(
      HashMap<String, Object> requestParams) async {
    bool connectionStatus = await ConnectivityStatus.isConnected();
    if (!connectionStatus) {
      return Left(Failure(
